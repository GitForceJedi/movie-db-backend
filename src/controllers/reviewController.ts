...
function validateReviewInput(input: any): input is ReviewInput {
  return (
...
export const createReview = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO "Review" ("movieId", "userId", rating, comment, stars) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [movieId, req.user.id, rating, comment, stars]
    );
...
export const getFilteredReviews = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (movieId) {
      params.push(parseInt(movieId as string));
